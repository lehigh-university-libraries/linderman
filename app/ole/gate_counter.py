...
def insert_count(timestamp, gate_name, alarm_count, alarm_diff, incoming, incoming_diff, outgoing, outgoing_diff):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO lib_gate_counts VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
            (timestamp, gate_name, alarm_count, alarm_diff, incoming, incoming_diff, outgoing, outgoing_diff)
        )
        conn.commit()
...
def update_gate_count(gate_url, gate_name):
    ...
    try:
        ...
        # Insert record
        insert_count(timestamp, gate_name, alarm_count, alarm_diff, incoming, incoming_diff, outgoing, outgoing_diff)
        
        logger.info(f"{gate_name}: alarm={alarm_count}({alarm_diff:+d}), in={incoming}({incoming_diff:+d}), out={outgoing}({outgoing_diff:+d})")
    ...
    while True:
        try:
            for i, url in enumerate(GATE_URLS):
                gate_name = 'FM South gate' if 'south' in url.lower() else 'FM West gate' if 'west' in url.lower() else f'Gate {i+1}'
